...
app.post('/api/add', (req: Request, res: Response) => {
  const { values, session, text } = req.body as { values?: number[], session?: string, text?: string };
  ...
  if (fs.existsSync(MAIN_FILE)) {
    try { main = JSON.parse(fs.readFileSync(MAIN_FILE, 'utf-8')); } catch {}
  }
...
app.post('/api/merge', (req: Request, res: Response) => {
  ...
  ensureDir(DATA_DIR);
  const rows: { session_id: string, multiplier: number }[] = [];
      ...
      try {
        ...
        if (Array.isArray(arr)) {
          for (const r of arr) {
            ...
            if (!isNaN(m) && m >= 1) rows.push({ session_id: String(r.session_id || path.basename(inPath)), multiplier: m });
          }
        }
      } catch {}
    }
...
function readMainValues(): number[] {
  try {
    const txt = fs.readFileSync(MAIN_FILE, 'utf-8');
    const arr = JSON.parse(txt);
    if (Array.isArray(arr)) return arr.map((r: any) => Number(r.multiplier)).filter((v: number) => !isNaN(v) && v >= 1);
  } catch {}
  return [];
}
function writeAppendCrash(value: number, sessionId = 'real') {
  ensureDir(DATA_DIR);
  let mainArr: any[] = [];
  if (fs.existsSync(MAIN_FILE)) {
    try { const txt = fs.readFileSync(MAIN_FILE, 'utf-8'); mainArr = JSON.parse(txt); } catch {}
  }
  if (!Array.isArray(mainArr)) mainArr = [];
  mainArr.push({ session_id: sessionId, multiplier: value });
  fs.writeFileSync(MAIN_FILE, JSON.stringify(mainArr));
}

app.post('/api/add-crash', (req: Request, res: Response) => {
  const { value } = req.body as { value?: number | string };
  let v = 0;
  if (typeof value === 'string') v = parseFloat(value.replace(/x$/i, '').trim());
  else if (typeof value === 'number') v = value;
  if (!v || Number.isNaN(v) || v < 1) return res.status(400).json({ error: 'Valor inválido, debe ser >= 1' });
  try { writeAppendCrash(Number(v.toFixed(2))); } catch {}
...
app.get('/api/summary', (req: Request, res: Response) => {
  ...
  const pge: Record<string, number> = {};
...
app.get('/api/sim-stream', (req: Request, res: Response) => {
  ...
  function sampleCrash(): number {
    const sample = readMainValues();
    if (sample.length === 0) {
      const xm = 1.0; const alpha = 2.0; // Pareto fallback
      const u = Math.random();
  ...
  const interval = setInterval(() => {
    ...
    if (x >= R) {
      ...
      try { writeAppendCrash(R, id); } catch {}
      R = sampleCrash();
      x = 1.0;
    } else {
...
app.get('/api/stream', (req: Request, res: Response) => {
  ...
  function survivalAt(x: number, sample: number[]): number {
    const n = sample.length;
    if (n === 0) return 0;
    let c = 0;
    for (const v of sample) if (v >= x) c++;
    return c / n;
  }
  ...
  const interval = setInterval(() => {
    ...
    x += step;
    if (x > 1000) x = 1.0; // wrap defensively
  }, 200);
...
app.get('/api/pf-stream', (req: Request, res: Response) => {
  res.setHeader('Content-Type', 'text/event-stream');
  res.setHeader('Cache-Control', 'no-cache');
  res.setHeader('Connection', 'keep-alive');
  res.flushHeaders?.();
  ...
  function hmacHex(serverSeed: string, clientSeed: string, n: number): string {
    const h = crypto.createHmac('sha256', Buffer.from(serverSeed, 'utf-8'))
      .update(Buffer.from(`${clientSeed}${n}`, 'utf-8'))
      .digest('hex');
    return h;
  }
  function toUniform(hex: string): number {
    const head = hex.slice(0, 13);
    const num = parseInt(head, 16);
    const denom = Math.pow(16, head.length);
    return num / denom;
  }
  function crashR(serverSeed: string, clientSeed: string, n: number, houseEdge: number): number {
